﻿﻿class Visualizer{
    ...
    static drawLevel(context,level,left,top,width,height,outputLabels){
        ...
        for(let i=0;i<inputs.length;i++){
            ...
            context.beginPath();
            context.arc(x,bottom,node_radius,0,Math.PI*2);
            context.fillStyle="black";
            context.fill();
            context.beginPath();
            context.arc(x,bottom,node_radius*0.6,0,Math.PI*2);
        ...
        for(let i=0;i<outputs.length;i++){
            ...
            context.beginPath();
            context.arc(x,top,node_radius,0,Math.PI*2);
            context.fillStyle="black";
            context.fill();
            context.beginPath();
            context.arc(x,top,node_radius*0.6,0,Math.PI*2);
            context.fillStyle=getRGBA(outputs[i]);
            context.fill();

            context.beginPath();
            context.lineWidth=2;
            context.arc(x,top,node_radius*0.8,0,Math.PI*2);
            ...
            if(outputLabels[i]){
                ...
                context.font=(node_radius*1.5)+"px Arial";
                context.fillText(outputLabels[i],x,top+node_radius*0.1);
                context.lineWidth=0.5;
                context.strokeText(outputLabels[i],x,top+node_radius*0.1);
            }
        }
    }
